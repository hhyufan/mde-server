...
2. Keep **Root Directory** set to `.` because this is a standalone repository.
...
The desktop client persists `path -> fileId` mappings locally so the same file always maps to the same UUID across runs.
...
- The server limits JSON request bodies to `5mb` in every environment.
