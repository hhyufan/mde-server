...
export class SyncService implements OnModuleInit {
  ...
  private pickConfigFields(config: Record<string, any>): Record<string, any> {
    return {
      ...
      previewFontSize: config.previewFontSize ?? config.fontSize ?? 14,
      fontFamily: config.fontFamily ?? 'JetBrains Mono',
      lineHeight: config.lineHeight ?? 24,
      previewLineHeight: config.previewLineHeight ?? config.lineHeight ?? 24,
      previewZoomSync: config.previewZoomSync ?? true,
